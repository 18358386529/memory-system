...
7. **kiwi-mem 自身使用 PostgreSQL(`asyncpg`)存储 embedding 向量**(`memories.embedding` 为 `TEXT` 列、`mem_scenes.embedding` 为 `JSONB` 列,详见清单4报告"发现的冲突与待人工裁决项"第7条),模型默认值为 `openai/text-embedding-3-small`,与 BGE-M3/Gemini 均不同。这是 kiwi-mem 自身既有的独立技术选型,并未出现在 haven-ombre/ombre-brain/xinchao-nian 三个仓库中,不构成对约束4的直接违反;但由于 kiwi-mem 被定位为"算法参考",记忆系统2.0团队在参考其 RRF 混合检索思路时,需要明确该参考**不包含**"引入 PostgreSQL 作为向量存储"这一层面,避免团队成员误以为"参考 kiwi-mem"等同于"可以把 PostgreSQL/向量表引入 haven-ombre"。此为提醒性质的边界澄清,请人工在制定迁移范围文档时明确写清楚。
